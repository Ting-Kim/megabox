...
<title>지난이벤트 < 이벤트 | 라이프씨어터, 메가박스</title>
<script
...
<body>
	...
	<div class="body-wrap">
		<section id="layer_login_select" class="modal-layer"
			...
			<div class="wrap"
				style="width: 425px; height: 365px; top: 200px; left: 37%;">
										...
										<form id="loginForm" action="<%=path%>/login.do" method="post">
											<div class="login-input-area">
												<input autocomplete="off" id="ibxLoginId" maxlength="20"
													name="id" type="text" placeholder="아이디" title="아이디를 입력하세요"
													class="input-text strTrim" <%if(cookieVal!=null){%>
													value="<%=cookieVal%>" <%} %>> <input
													autocomplete="off" id="ibxLoginPwd" maxlength="20"
													name="pwd" type="password" placeholder="비밀번호"
													title="비밀번호를 입력하세요" class="input-text mt15">
												<div class="alert"></div>
												<div class="chk-util">
													<div class="left">
														<input id="chkIdSave" name="chkIdSave" type="checkbox"
															<%if(cookieVal!=null&&cookieVal!=""){%> checked="checked"
															<%}%>> <label for="chkIdSave">아이디 저장</label>
													</div>
												</div>
												<button id="btnLogin" type="submit"
													class="button purple large btn-login">로그인</button>
												<div class="link">
													<a href="<%=path%>/user-find.do?find=id"
														title="ID/PW 찾기 선택">ID/PW 찾기</a> <a
														href="<%=path%>/join.do" title="회원가입 선택">회원가입</a> <a
														href="<%=path%>/nonMem.do" title="비회원 예매확인 선택">비회원
														예매확인</a>
												</div>
											</div>
										</form>
			...
			<nav id="gnb" class="">
				<ul class="gnb-depth1">
					...
					<li><a href="<%=path %>/event.do" class="gnb-txt-event"
						...
						<div class="gnb-depth2">
							<ul>
								<li><a href="<%=path %>/event.do" title="진행중 이벤트">진행중
										이벤트</a></li>
		...
		<div class="container">
			...
			<div class="page-util">
				<div class="inner-wrap">
					<div class="location">
						<span>Home</span> <a href="javascript:void(0)"
							onclick="NetfunnelChk.aTag('EVENT_LIST','/event');return false;"
							title="이벤트 메인 페이지로 이동">이벤트</a> <a href="/event/end"
							title="지난 이벤트 페이지로 이동">지난 이벤트</a>
			...
			<div id="contents">
				...
				<div class="inner-wrap">
					...
					<ul class="dot-list">
						...
						<!-- 로그인 전 -->
						<li>응모하신 이벤트의 당첨 여부는 당첨자발표의
							<button type="button" class="reset tooltip click">
								<strong class="font-gblue">나의 응모결과 확인</strong> <span class="ir"
									data-width="235"> <span class="cont-area"> <span
										class="login-alert-tooltip"> 로그인이 필요한 서비스 입니다.<br /> <a
											id="moveLogin" href="#layer_login_common"
											class="btn-modal-open" w-data='850' h-data='484'
											title="로그인 바로가기">로그인 바로가기 <i
												class="iconset ico-arr-right-green"></i></a>
									</span>
								</span>
								</span>
							</button> 을 통해 확인하실 수 있습니다.
						</li>
						<!--// 로그인 전 -->
				...
				<div id="boardSearch" class="inner-wrap" style="">
					...
					<div class="board-list-util mt0">
						...
						<div class="board-search">
							<input type="text" id="search" title="검색어를 입력해 주세요."
		 ...
		 $('#searchBtn').on('click', function() {
            ...
            window.location.href="<%=path%>/event/past.do?type="+6+"&search="+search; 			
				...
				<div class="inner-wrap" id="pastlist">


					<div class="event-list finish ">


						<input type="hidden" id="totCount" name="totCount" value="1">
						<ul id="ulid">
							<c:forEach items="${list}" var="dto">
								<li><a href="<%=path %>/event/info.do?num=${dto.seq_event}"
									data-no="8144" data-netfunnel="N" class="eventBtn"
									title="${dto.title } 상세보기"> <c:if
											test="${dto.seq_etype ==1}">
											<p class="cate">메가 Pick</p>
										</c:if> <c:if test="${dto.seq_etype ==2}">
											<p class="cate">영화</p>
										</c:if> <c:if test="${dto.seq_etype ==3}">
											<p class="cate">극장</p>
										</c:if> <c:if test="${dto.seq_etype ==4}">
											<p class="cate">제휴/할인</p>
										</c:if> <c:if test="${dto.seq_etype ==5}">
											<p class="cate">시사회/무대인사</p>
										</c:if> <!--<p class="img"><img src="../../../static/pc/images/event/@img-event-list-megabox.jpg" alt="" /></p>-->
				...
				<!-- 더보기 -->
				<div class="btn-more v1" style="" id="divajax">
					<button type="button" class="btn" id="btnajax">
						더보기 <i class="iconset ico-btn-more-arr"></i>
					</button>
				</div>
					...
					success:function(data){
						...
						if (endNum>0) {
							for (var i = 0; i < data.list.length; i++) {
								/* var idx=Number(endNum)+Number(i)+1; */
								var page = data.list[i].seq_event;
								console.log(page);
								addListHtml +="<ul>";
								addListHtml +="<li>";
								addListHtml +="<a href='<%=path %>/event/info.do?num="+data.list[i].seq_event+"'data-no='8144' data-netfunnel='N' class='eventBtn' title="+data.list.title+ "상세보기'>";
								
								switch (data.list[i].seq_etype) {
								case 1:
								addListHtml +="<p class='cate'>메가 Pick</p>"; break;
								case 2:
								addListHtml +="<p class='cate'>영화 </p>"; break;
								case 3:
								addListHtml +="<p class='cate'>극장 </p>"; break;
								case 4:
								addListHtml +="<p class='cate'>제휴/할인 </p>"; break;
								case 5:
								addListHtml +="<p class='cate'>시사회/무대인사</p>"; break;
								default:
									break;
								}
								addListHtml +="<p class='img'>";
								addListHtml += "<img src='<%=path %>"+ data.list[i].img_addr+"'alt='"+ data.list.title+ "'onerror='noImg(this);'>";
								addListHtml += "</p>";
								addListHtml += "<p class='tit'>"+ data.list[i].title+ "</p>";
								addListHtml += "<p class='date'>"+ data.list[i].start_date_ajax+ " ~ "+ data.list[i].end_date_ajax+ "</p>";
								addListHtml += "</a>";
								addListHtml += "</li>";
								addListHtml += "</ul>";
		...
		<div class="quick-area" id="divtop" style="display: none;">
			<a href="" class="btn-go-top" id="btn_top" title="top"
				style="position: fixed;">top</a>
		</div>
	...
	$(function() {
		...
		<!--// footer -->
		<script src="<%=path%>/css/eventscript.js">
