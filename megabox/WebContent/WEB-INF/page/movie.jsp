...
<!-- --------------------------------------------------------------------------- -->
			...
			<div id="contents" class="">
				...
				<div class="inner-wrap">
					...
					<div class="tab-list fixed">
						<ul id="topMenu">
							<li <% if(title.equals("박스오피스")){%>class="on" <%}%>><a href="<%=path %>/movie.do" title="박스오피스 탭으로 이동">박스오피스</a></li>
							<li <% if(title.equals("상영예정작")){%>class="on" <%}%>><a href="<%=path %>/movie.do?name=comingsoon" title="상영예정작 탭으로 이동" >상영예정작</a></li>
							<li <% if(title.equals("필름소사이어티")){%>class="on" <%}%>><a href="<%=path %>/movie.do?name=film" title="필름소사이어티 탭으로 이동">필름소사이어티</a></li>
							<li <% if(title.equals("클래식소사이어티")){%>class="on" <%}%>><a href="<%=path %>/movie.do?name=classic" title="클래식소사이어티 탭으로 이동">클래식소사이어티</a></li>
						</ul>
					</div>
					<!-- movie-list-util -->
					<div class="movie-list-util mt40">
						<%if(title.equals("박스오피스")){%>
						...
						<p class="no-result-count">
							<strong id="totCnt">${fn:length(list)}</strong>개의 영화가 검색되었습니다.
						</p>
						</c:if>
					...
					<div class="movie-list">
						...
						<c:forEach var="dto" items="${list}">
							...
							<%}%>
								<img src="<%=path%>/img/poster/${dto.num}/${dto.num}.jpg" alt="이미지" class="poster lozad">
								<div class="curation">
									<p class="film" style="display: none">필름 소사이어티</p>
									<p class="classic" style="display: none">클래식 소사이어티</p>
								</div>
								<div class="movie-score">
									<a href="<%=path%>/movie-detail.do?movie=${dto.num}" class="wrap movieBtn" title="${dto.name} 상세보기">
							...
							<div class="tit-area">
								...
								<p title="영화명" class="tit">${dto.name}</p>
							</div>
							<div class="rate-date">
								<span class="rate">예매율 0%</span> <span class="date">개봉일 ${dto.date}</span>
							</div>
							<div class="btn-util">
								<button type="button" class="button btn-like" data-no="20021300">
									<i title="보고싶어 안함" class="iconset ico-heart-toggle-gray intrstType"></i> <span>0 명</span>
								</button>
								<div class="case movieStat4">
									<a href="<%=path%>/booking.do?movie=${dto.num}" class="button purple bokdBtn" title="영화 예매하기">예매</a>
								</div>
							</div>
						 </li>
						</c:forEach>
		...
		<footer id="footer">
			<div class="footer-top">
				<div class="inner-wrap">
					<ul class="fnb">
						<li><a href="<%=path%>/company.do" title="회사소개 페이지로 이동">회사소개</a></li>
						<li><a href="<%=path%>/company.do" title="인재채용 페이지로 이동">인재채용</a></li>
						<li><a href="<%=path%>/company.do" title="사회공헌 페이지로 이동">사회공헌</a></li>
						<li><a href="<%=path%>/company.do" title="제휴/광고/부대사업문의 페이지로 이동">제휴/광고/부대사업문의</a></li>
						<li><a href="<%=path%>/company.do" title="이용약관 페이지로 이동">이용약관</a></li>
						<li class="privacy"><a href="#" title="개인정보처리방침 페이지로 이동">개인정보처리방침</a></li>
						<li><a href="<%=path%>/company.do" target="_blank" title="윤리경영 페이지로 이동">윤리경영</a></li>
					</ul>

					<a href="<%=path%>/company.do" class="btn-looking-theater" title="극장찾기">
