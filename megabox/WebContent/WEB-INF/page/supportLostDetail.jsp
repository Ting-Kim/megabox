...
<!-- 나의 메가박스 -->
				...
				<div class="table-wrap">
					<div class="board-view">
						...
						<div class="info">
							...
							<p><strong class="txt">${list.branch}</strong></p>
							<p><span class="txt">${list.tel}</span></p>
							<p><span class="txt">${list.email}</span></p>
							<p><span class="txt"> ${list.regidate}</span></p>
							<p>
								
									
									<span class="txt">${list.isapplied}</span>
								
							</p>
						</div>

						
							<textarea rows="5" cols="30" readonly="" class="input-textarea view" style="border:none;">${list.content}</textarea>
					<div class="cont">	</div>
						<!-- !!!!!!!!!!!!!!!!!!!!!!!!!!답변영역!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! -->
						
					 <c:if test="${auth.type eq 9 }">
						<div class="reply">
						
						<input type="text" placeholder="댓 글 입 력"  id="contentLost"  val=""
						style="text-align:center; font-size:30px;   width:800px; height:300px; letter-spacing: 5px"/>
					<br>
								
					<div class="btn-group pt40">
					<button" id="insertbtn" class="button large" title="목록">입력</button>
				</div>
						</div>
					</c:if>	
					<script>
					var seq_lost = ${list.seq_lost};
					
							$('#insertbtn').on('click', function(){
	
								
								 var contents= $("#contentLost").val();

 								$.post('<%=path%>/support/lost_reply_write.aj?seq_lost='+seq_lost+'&contents='+contents , function (data) {		
 											
 										var seqWrite=data.data.lost_reply_seq;
 																  
 									 str=	'<div class="cont review" id ="replys">'+data.data.content ;
					...
					<!-- 세션검사해서 운영자일때	출력 -->
						...
						<c:if test="${ not  dto.isEmpty()}">
								<c:forEach items="${dto}" var="dto" >		
						
									<div class="cont review" id ="replys">${dto.content}
									<c:if test="${auth.type eq 9 }">
									<button class="deleteBtn"  value="${dto.lost_reply_seq}" >삭제</button></c:if> </div><br>
	...
	<script>
							...
							$(document).on('click','.deleteBtn' ,function(e){
								var seq_lost = ${list.seq_lost}; //글번호
								var seqWrite=(this).value; //댓글번호
								
							 $('#reply').empty(); 
						
							 $.ajax({
								 type:'post',
								 url:'<%=path%>/support/lost_reply_delete.aj?seqWrite='+seqWrite+'&seq_lost='+seq_lost ,
								  dataType : 'json',								  
								success: function(data){	
									 $.each(data , function(index, datas){				 							
 									 console.log(datas.content);
 									str=	'<div class="cont review" id ="replys">'+datas.content+'<c:if test="${auth.type eq 9 }">'
 									str+= '<button class="deleteBtn"  value= "'+datas.lost_reply_seq+'" >삭제</button></c:if> </div><br>';
										
 								 		console.log(datas.lost_reply_seq);
										$('#reply').append(str); 
					...
					<div class="btn-group pt40">
					<a href="<%=path%>/support/lostQna.do" id="listBtn" class="button large" title="목록">목록</a>
				</div>
				</div>
