		...
		<div class="inner-wrap">
			<div class="lnb-area">
				<nav id="lnb">
					...
					<ul>
						...
						<li><li class="on"><a href="<%=path %>/support/notice.do" title="공지사항">공지사항</a></li>
						<li><a href="<%=path %>/support/inquiry.do" title="1:1문의">1:1문의</a></li>
			...
			<div id="contents" class="">
				...
				<div class="tab-block mb30">
					...
					<ul>
						<li class="tabBtn on">
						<button type="button" class="btn tabBtn ajax" data-no="" title="전체공지 보기" value ="0"  >전체</button></li>
						
							<li class="tabBtn">
							<button type="button" class="btn ajax" data-no="81" title="메가박스 공지 보기"  value ="107"  >메가박스 공지</button></li>
						
							<li class="tabBtn">
							<button type="button" class="btn ajax" data-no="82" title="지점 공지 보기" value ="2" >지점 공지</button></li>
...
<!-- <p class="result-count"><strong>전체 <em class="font-gblue">몇개게~</em>건</strong></p> -->


<div>
<select id="loc" title="지역 선택" name =" loc"><option class="bs-title-option" value="0" id="loc">지역선택</option>					
...
<select id="theater" title="극장선택"><option class="bs-title-option" value="">극장선택</option>
	...
	$('#searchBtn').click();}
		...
		$('#searchBtn').on('click', function(e) {
				 ...
				 console.log(searchWord);
					...
					$.get('<%=path%>/suppport/notice/search.aj?searchWord='+searchWord  , function (data) {		
						console.log(data.data.list2);
						
				 		 $.each(data.data.list2, function(index, datas){	
							console.log(data.data);
							
							var str = 	"<tr><td>"+datas.rownum+" </td>"
							str +=	"<td>" + datas.branch+ "</td>"
							str +=		"<td>" + datas.npart +"</td>"
							str +=	"<th>"
								
								
							str +=	"<a href='/megabox/support/noticePage.do?seqNotice="+datas.seq_notice+"'class='moveBtn' data-no='10121' title='공지사항 상세보기'>"	
								
							str +=		" <span class='font-green'></span>"+datas.title
							str +=	 "<span class='font-green'></span> "				
							str +=	" <span class='font-green'></span>"
							str +=	  " <span class='font-green'></span>"
							str +=	  "</a> "
							str +=	  "</th> "
							str +=	  "<td>"+datas.regidate+ "</td>"
							str +=	 "</tr>"
		...
		$('select[id=loc]').change(function() {
			...
			$.get('<%=path%>/suppport/notice/loc.aj?loc='+location  , function (data) {				
			...
			$.get('<%=path%>/suppport/notice/locList.aj?theater='+theater , function (data) {				
...
//	$('.btn tabBtn').parent().addClass('on');
	...
	$('.ajax').click(function (event){
		...
		$.get('<%=path%>/suppport/notice/tab.aj?seq='+event.target.value, function (data) {
	...
	$(this).addClass('on').siblings().removeClass('on');
				...
				<div class="table-wrap">
					<table class="board-list">
						...
						<c:if test="${ not  notice.isEmpty()}">
								<c:forEach items="${notice.list2}" var="dto">
					
					
						<tr><td>${dto.seq_notice}</td>
						<td>${dto.branch}</td>
						<td>${dto.npart}</td>
						<th>
						
						<a href="<%=path%>/support/noticePage.do?seqNotice=${dto.seq_notice}" class="moveBtn" data-no="10121" title="공지사항 상세보기">
						
						 <span class="font-green" ></span>${dto.title}
            ...
            <c:forEach var="pageNum" begin="1"   end="${ notice.pageTotalCount }">
               ...
               <c:if test="${ not (pageNum eq notice.currentPageNumber) }"> 	
                  <a href="<%=path %>/support/notice.do?page=${ pageNum }&seq=0">${ pageNum }</a>
               </c:if>
            </c:forEach>
       			  </div>
