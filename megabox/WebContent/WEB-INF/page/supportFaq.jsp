...
<!-- 나의 메가박스 -->
    <div id="layer_mymega" class="header-layer layer-mymege">
      ...
      <c:if test="${empty sessionScope.auth}">
		...
		<body>
			<input type="hidden" id="idx" name = "searchWord" value="" />
			...
			<div class="container no-padding-bottom has-lnb">
				<div class="page-util">
					<div class="inner-wrap">
						<div class="location">
							<span>Home</span> <a href="<%=path %>/support.do"
								title="고객센터 페이지로 이동">고객센터</a> <a
								href="<%=path %>/support/faq.do" title="자주 묻는 질문 페이지로 이동">자주
								묻는 질문</a>
						</div>
				...
				<div class="inner-wrap">
					<div class="lnb-area">
						<nav id="lnb">
							...
							<ul>
								<li><a href="<%=path %>/support.do" title="고객센터 홈">고객센터
										홈</a></li>
								<li class="on"><a href="<%=path %>/support/faq.do" id="faqbtn"
									title="자주 묻는 질문">자주 묻는 질문</a></li>
								<li><a href="<%=path %>/support/notice.do" title="공지사항" >공지사항</a></li>
								<li><a href="<%=path %>/support/inquiry.do" title="1:1문의">1:1문의</a></li>
								<li><a href="<%=path %>/support/rent.do"
									title="단체관람 및 대관문의">단체관람 및 대관문의</a></li>
								<li><a href="<%=path %>/support/lost.do" title="분실물 문의">분실물
										문의</a></li>
								<li><a href="<%=path %>/support/terms.do" title="이용약관">이용약관</a></li>
								<li><a href="<%=path %>/support/privacy.do"
									title="개인정보처리방침">개인정보처리방침</a></li>
								<li><a href="<%=path %>/support/screenrule.do"
					...
					<div id="contents">
						...
						<form action ="<%=path %>/support/faq.do"  method ="get">
							<div class="input-area">
								<label for="search01" class="label">빠른검색</label>

								<!-- 2019-01-21 검색 부분 변경 -->
								<div class="board-search w460px">
									<input type="search" id="searchTxt" name = "searchWord" title="검색어를 입력해 주세요."
...
<%-- onClick="location.href =<%=path %>/support/faq.do" --%>
							
								
								<li><button type="button" class="btn tabBtn ajax" id="totalTab" value = "0" onClick="window.location.reload(true);">전체</button>
								</li>
								
								<li><button type="button" class="btn tabBtn ajax"  value="1">영화예매</button>
								</li>
								<li><button type="button" class="btn tabBtn ajax"    value="2">할인혜택</button>
								</li>
								<li><button type="button" class="btn tabBtn ajax"  value="3">결제수단/관람권</button>
								</li>
								<li><button type="button" class="btn tabBtn ajax" value="4">멤버십</button>
								</li>
								<li><button type="button" class="btn tabBtn ajax"  value="5">VIP</button>
								</li>
								<li><button type="button" class="btn tabBtn ajax"  value="6">극장</button>
								</li>
								<li><button type="button" class="btn tabBtn ajax" value="7">특별관</button>
								</li>
								<li><button type="button" class="btn tabBtn ajax"  value="8">스토어</button>
								</li>
								<li><button type="button" class="btn tabBtn ajax"  value="9" id="sss">홈페이지/모바일</button></li>
...
<script>
 ...
 $(function(){
	...
	$('.ajax').click(function (event){
		...
		$.get('<%=path%>/suppport/faq.aj?seq='+event.target.value, function (data) {
			
	 
			
			var d_length = data.messageTotalCount;
		
			console.log(data.d_length);
			
			$("#totalCnt").text(d_length);

			var category = event.target.innerText;
			
			$("#totalT").text(category);
		...
		function puc(data,category){
			 $.each(data.list , function(index, datas){
					
			 	
						
				var str =	'<li><div class="qut" >'
					str	+=	'<a href="#"><p class="tit">'
					str	+=	'<span class="font-green">'+category+'</span></p>'
					str	+=	'<p class="txt"><span class="font-green" id ="ajtitle">'
					str+=	'</span>'+datas.TITLE+'</p></a>'			
					str+='</div><div class="awn"> <p class="cont"><span style="font-size: 14px;">'+datas.CONTENT+'<br></p>'
					str+=	'<br> </div></li>';	 		''
					
					
					$('#dataToggle').append(str);
...
$('li').click(function (){
	...
	$(this).addClass('on').siblings().removeClass('on');
						...
						<div class="faq-list-box">
							<p class="reset mb10">
								<strong><span id="totalTitle"></span>
								<span	class="font-green" id="totalT">전체</span>
								 <span class="font-green" id="totalCnt">42</span>건</strong>
							</p>
							...
							<div class="faq-list">
								<ul  id="dataToggle" >
									...
									<c:if test = "${veiw.isEmpty()}">
								등록된 메세지가 없습니다
								</c:if>

									<c:if test="${ not view.isEmpty()}">
										<c:forEach items="${view.list}" var="dto">
								
							
											<li><div class="qut">
													<a href=""><p class="tit">
															<span class="font-green">${dto.FAQ_PART }</span>
														</p>

														<p class="txt">
															<span class="font-green" id="ajtitle"> </span>${dto.TITLE}
														</p> </a>
												...
												<div class="awn">
													...
													<p class="cont">
														<span style="font-size: 14px;">${dto.CONTENT}<br>
													</p>
													<br>
													</div>
													</li>
            ...
            <c:forEach var="pageNum" begin="1"   end="${ view.pageTotalCount }">
               ...
               <c:if test="${ not (pageNum eq view.currentPageNumber) }">
                  ...
                  <a href="<%=path %>/support/faq.do?page=${ pageNum }&searchWord=${view.searchWord}">${ pageNum }</a>
               </c:if>
            </c:forEach>
         </div>
