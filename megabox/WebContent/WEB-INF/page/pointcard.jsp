...
<script src="<%=path%>/js/megabox.js"></script>
		<title>포인트/기타제휴 < 혜택 | 라이프씨어터, 메가박스</title>
</head>
...
function fn_selectCardInfo(){
	var data = null;
	$.ajax({
		url: "/megabox/benefit/cardlist.ajax?cardKindCd=CKC03",
		type: "POST",
		contentType: "application/json;charset=UTF-8",
		dataType: "json",
		data: data, //CKC03: 포인트/제휴
...
function fn_selectList(selectList) {
	...
	for (var i=0; i<selectList.length; i++ ) { //검색결과 있는 만큼 반복
		if(cardPartrNm != selectList[i].CORPORATION){
				...
				htmlText +="<span id=\""+ selectList[i].CORPORATION +"\">" + selectList[i].CORPORATION + "</span>"; //하위 카드종류 ex: 우리카드
				htmlText +="</h3>";
...
/* 
			...
			htmlText +="<div class=\"btn\"><a href=\""+ selectList[i].CARD_URL +"\" target=\"_blank\" title=\""+ dataList[i].linkRplWordsCn +"\" class=\"button purple\">자세히 보기</a></div>"; //카드사링크이동버튼
			} */
			htmlText +="</div></div></li>";
		}
				...
				<div class="inner-wrap mt40">
					...
					<div class="tab-list fixed" id="tab-list">
						<ul>
							...
							<li class="on"><a href="<%= path %>/benefit/discount.do?part=3" id="tab_point" title="포인트 기타제휴 탭으로 이동">포인트/기타제휴(${dto.CKC03CNT})</a></li>
