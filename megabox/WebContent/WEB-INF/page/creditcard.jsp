...
<title>신용카드 < 혜택 | 라이프씨어터, 메가박스</title>
</head>
...
$(function(){
	...
	fn_move();
	// fn_animation(); //배너 애니메이션		
	fn_selectCardInfo('','Y'); 


});

	function fn_selectCardInfo(str, sw){
		var data = null;
		console.log(str);
		$.ajax({
			url: "/megabox/benefit/cardlist.ajax?cardKindCd=CKC02&cardPartrCd="+str,
			type: "POST",
			contentType: "application/json;charset=UTF-8",
			dataType: "json",
			data: data, //CKC02 : 신용카드
			success: function (data, textStatus, jqXHR) {
				var telecom = data.countMap.count.CKC02CNT; //통신사
				var credit =  data.countMap.count.CKC01CNT; //신용카드
				var point =   data.countMap.count.CKC03CNT; //포인트
				...
				if(sw=="Y"){
					fn_selectCardList(data.cardCoList);
				}
				 fn_selectList(data.selectList);
			},
			complete: function() {
	        	$("div.tab-sorting").removeClass("on");
	        	if (str == '') {
	            	$("#CPCT00").attr("class","on");
	        	} else {
	            	$("#"+str).attr("class","on");
	        	}
	...
	function fn_selectCardList(result) {
		...
		for(var i = 0; i < result.length; ++i) {
			...
			$('#tab-creditCardlist').append("<button type=\"button\" onclick=\"fn_selectCardInfo('"+result[i].CORPORATION_ID+"',  '' )\" id =\""+result[i].CORPORATION_ID +"\">" + result[i].CORPORATION + "</button>");
	...
	/*카드 검색 리스트 조회 함수*/
	function fn_selectList(selectList) {
		var htmlText = "";
		var cardPartrNm = "";
		console.log(selectList);


		for (var i=0; i< selectList.length; i++ ) { //검색결과 있는 만큼 반복

				if(cardPartrNm != selectList[i].CORPORATION){
					...
					htmlText +="<span> " + selectList[i].CORPORATION + "</span>"; //하위 카드종류 ex: 우리카드
					htmlText +="</h3>";
					cardPartrNm = selectList[i].CORPORATION;
					htmlText +="<div class=\"list\"><ul>";
				}

				htmlText +="<li><div class=\"table\"><div class=\"img\">";

				if(selectList[i].CARD_IMAGE != ""){ //카드이미지 가로형
					htmlText += "<img src=\"<%= path %>"+selectList[i].CARD_IMAGE+" \"alt=\""+selectList[i].imgRplWordsCn +"\" onerror=\"noImg(this);\"/></div>"; //카드이미지 및 대체텍스트
				}
				htmlText +="<div class=\"cont\">";
				htmlText +="<div class=\"top\"><div class=\"name\">"+selectList[i].CARD_NAME+"</div>"; //카드이름

				if(selectList[i].CARD_URL != null ){ //카드할인여부:현장할인
					htmlText +="<div class=\"sale\"><span class=\"offline\">현장할인</span></div>";
				}
				if(selectList[i].CARD_URL != null ){//카드할인여부:인터넷할인
					htmlText +="<div class=\"sale\"><span class=\"online\">인터넷할인</span></div>";
				}
				htmlText +="</div>";
				htmlText +="<ul class=\"dot-list\">"+ selectList[i].CARD_CONTENT +"</ul>"; //카드설명내용

				if(selectList[i].CARD_URL !=null){
				htmlText +="<div class=\"btn\"><a href=\""+ selectList[i].CARD_URL +"\" target=\"_blank\" title=\""+ selectList[i].linkRplWordsCn +"\" class=\"button purple\">자세히 보기</a></div>"; //카드사링크이동버튼
				}
				htmlText +="</div></div></li>";
			}
		...
		document.getElementById("partnership-search-result").innerHTML= "";
		document.getElementById("partnership-search-result").innerHTML= htmlText;
				...
				<div class="inner-wrap">
					...
					<div class="partnership-count">
						<span title="현재 페이지" class="active">1</span> / <span
							title="전체 페이지" class="total">2</span>
					</div>

					<div class="partnership-util">
						<button id="btnPrevEvent" type="button" class="partnership-prev">이전
							이벤트 보기</button>
						<button id="btnNextEvent" type="button" class="partnership-next">다음
							이벤트 보기</button>
					...
					<div class="partnership-control">
						<button id="btnPrevEvent2" type="button" class="partnership-prev">이전
							이벤트 보기</button>
						<button id="btnNextEvent2" type="button" class="partnership-next">다음
							이벤트 보기</button>
					</div>
					...
					<div class="partnership-swiper">
						<div class="swiper-wrapper" id="swiper-wrapper-area">
							...
							<div class="cell swiper-slide">
								<a href="#" class="eventBtn" data-no="8355" data-netfunnel="N"
									title="[KB 랩비트 페스티벌 X 메가박스] 티켓 패키지 판매!! 상세보기">
									<p class="img">
										<img
							...
							<div class="cell swiper-slide">
								<a href="#" class="eventBtn" data-no="8487" data-netfunnel="N"
									title="[서울시민카드 X 메가박스] 영화&amp;콤보 할인 이벤트! 상세보기">
									<p class="img">
										<img
											src="https://img.megabox.co.kr/SharedImg/event/2020/06/30/aG7enF1RfSc0tuqHkhjcObFpekw3gY5R.jpg"
											alt="서울시민카드 영화&amp;amp;콤보 할인이벤트" onerror="noImg(this);" />
									</p>
									<div class="cont">
							...
							<div class="cell swiper-slide">
								<a href="#" class="eventBtn" data-no="7153" data-netfunnel=""
									title="T멤버십 VIP Pick 상세보기">
									<p class="img">
										<img
											src="https://img.megabox.co.kr/SharedImg/event/2020/02/04/7VVcQcPze1eYIuKc8ZjvRIEvKDeUTTMa.jpg"
											alt="" onerror="noImg(this);" />
									</p>
									<div class="cont">
							...
							<div class="cell swiper-slide">
								<a href="#" class="eventBtn" data-no="5405" data-netfunnel=""
									title="KT멤버십 VIP 무료 예매 상세보기">
									<p class="img">
										<img
											src="https://img.megabox.co.kr/SharedImg/event/2020/02/04/ICEWiqhHXgnL6SiCLcTo4bCeRzPXwPN7.jpg"
											alt="" onerror="noImg(this);" />
									</p>
									<div class="cont">
							...
							<div class="cell swiper-slide">
								<a href="#" class="eventBtn" data-no="6445" data-netfunnel=""
									title="현대카드 고객이라면, M포인트 장당 2000P 사용! 상세보기">
									<p class="img">
										<img
											src="https://img.megabox.co.kr/SharedImg/event/2020/02/04/aSjjQwUFJeJ1vtDwchfyaSGXpSDbnPpF.jpg"
											alt="" onerror="noImg(this);" />
									</p>
									<div class="cont">
					...
					<div class="more">
						<a href="javascript:void(0)" onclick="fn_goEvent()"
							title="이벤트 페이지로 이동">더 많은 제휴혜택 보기 <i
			...
			<div class="inner-wrap mt40">
				...
				<div class="tab-list fixed" id="tab-list">
					<ul>
						...
						<li class="on"><a href="<%= path %>/benefit/discount.do?part=1" id="tab_credit" title="신용카드 탭으로 이동"></a></li>
						<li><a href="<%= path %>/benefit/discount.do?part=2" id="tab_telecom" title="통신사 탭으로 이동"></a></li>
						<li><a href="<%= path %>/benefit/discount.do?part=3" id="tab_point" title="포인트 기타제휴 탭으로 이동"></a></li>
						<li><a href="<%= path %>/benefit/discount.do?part=4" id="tab_gift" title="상품권 관람권 탭으로 이동">상품권/관람권</a></li>
					</ul>
				</div>
				...
				<div class="partnership-page mt40">
					...
					<!-- 카드회사 버튼 리스트  -->
						<div class="tab-sorting mt30" id="tab-creditCardlist">
							<button type="button" class="" id="CPCT00" onclick="fn_selectCardInfo()">전체</button>
						</div>
					</div>
					<div class="partnership-search-result" id="partnership-search-result">
				</div>
			</div>
		</div>
	</div>
