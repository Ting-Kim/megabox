...
<link rel="stylesheet" href="<%= path%>/css/megabox.min.css" media="all" />
    ...
    <script src="<%=path%>/js/megabox.js"></script>
		<title>통신사 < 혜택 | 라이프씨어터, 메가박스</title>
</head>
...
$(function(){
  		...
  		// fn_animation();		 //제휴할인 이벤트 배너
	    fn_selectCardInfo(); //상단탭 건수설정

});

/*상단탭 카드 건수 및 통신사 카드 정보 조회*/
function fn_selectCardInfo(){
	var data = null;
	$.ajax({
		url: "/megabox/benefit/cardlist.ajax?cardKindCd=CKC01",
		type: "POST",
		...
		success: function (data, textStatus, jqXHR) {
			...
			// fn_move(''); //화면 앵커이동, 파라미터: 할인안내 페이지에서 셀렉트박스 선택 값
...
function fn_selectList(selectList) {
	...
	for (var i=0; i<selectList.length; i++ ) { //검색결과 있는 만큼 반복
		if(cardPartrNm != selectList[i].CORPORATION){
			...
			htmlText +="<span id=\""+ selectList[i].CORPORATION +"\">" + selectList[i].CORPORATION + "</span>"; //SKT, KT, U+
			htmlText +="</h3>";
		...
		if(selectList[i].CARD_NAME !=""){ //카드할인여부:현장할인
			htmlText +="<div class=\"sale\"><span class=\"offline\">현장할인</span></div>";
		}
		if(selectList[i].CARD_NAME !=""){//카드할인여부:인터넷할인
...
function fn_move(id){
    ...
    if(id != ""){ //할인안내 페이지에서 통신사>셀렉트박스 값을 선택했을 경우
	    offset = $('#'+id).offset().top-150; //해당 통신사로 앵커 이동
		$('html,body').animate({scrollTop : offset}, 300);

	}else{
		offset = $('#tab-list').offset();
			...
			<div class="container">
				<div class="page-util">
					<div class="inner-wrap">
						<div class="location">
							<span>Home</span> <a href="<%= path %>/benefit/membership.do"
								title="혜택 메인 페이지로 이동">혜택</a> <a
								href="<%= path %>/benefit/discount.do" title="제휴할인 페이지로 이동">제휴/할인</a>
							<a href="<%= path %>/benefit/membership.do?name=membership"
								title="할인안내 페이지로 이동">할인안내</a>
						</div>
					...
					<div class="partnership-slider">
						<div class="inner-wrap">
							...
							<div class="partnership-control">
								<button id="btnPrevEvent2" type="button"
									class="partnership-prev">이전 이벤트 보기</button>
								<button id="btnNextEvent2" type="button"
									class="partnership-next">다음 이벤트 보기</button>
							</div>
							...
							<div class="more">
								<a href="<%= path %>" onclick="fn_goEvent()" title="이벤트 페이지로 이동">더 많은 제휴혜택 보기 <i class="iconset ico-arr-right-green"></i>
								</a>
					...
					<div class="inner-wrap mt40">
						...
						<div class="tab-list fixed" id="tab-list">
							<ul>
								...
								<li class="on"><a href="<%= path %>/benefit/discount.do?prat=2" id="tab_telecom" title="통신사 탭으로 이동">통신사(${dto.CKC02CNT})</a></li>
						...
						<div class="search-card v2">
							<div class="tit-area">
								...
								<p class="txt">소지하고 계신 카드를 검색해 보세요. 할인 및 혜택정보를 빠르게 확인할 수
									있어요.</p>
							</div>
						</div>
							<!--// search-card -->
