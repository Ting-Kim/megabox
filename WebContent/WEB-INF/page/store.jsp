...
<%@page import="megabox.mvc.model.StoreInfoDTO"%>
...
<link rel="stylesheet" href="css/megabox.min.css" media="all" />
		...
		<title>새로운상품 < 스토어 | 라이프씨어터, 메가박스</title>
		<meta property="name"			id="metaTagTitle"	content="새로운상품 &lt; 스토어"/>
		<meta property="description"	id="metaTagDtls"	content="메가박스 스토어의 다양한 상품을 만나보세요."/>
		...
		<meta property="og:url" 		id="fbUrl"		content="https://www.megabox.co.kr/store" />
		<meta property="og:title" 		id="fbTitle"	content="새로운상품 &lt; 스토어" />
		<meta property="og:description"	id="fbDtls"		content="메가박스 스토어의 다양한 상품을 만나보세요."/>
...
<div class="container">
    ...
    <div class="page-util">
        <div class="inner-wrap">
            <div class="location">
                <span>Home</span>
                <a href="store.do?group=0" title="스토어">스토어</a>
            </div>
    ...
    <div id="storeMainList"></div>
		 ...
		 <a href="store.do?group=0">새로운 상품</a>
		 <a href="store.do?group=1">메가티켓</a>
		 <a href="store.do?group=2">메가찬스</a>
		 <a href="store.do?group=3">팝콘/음료/굿즈</a>


   <c:if test="${ not empty list }">
	 <c:forEach items="${list}" var="dto">
	 
		 <h3>${dto.SEQ_PRODUCT}</h3>
		 <h3>${dto.SEQ_GROUP}</h3>
		 <h3>${dto.PRODUCT_NAME}</h3>
		 <h3>${dto.DESCRIPTION}</h3>
		 <h3>${dto.PRODUCT_CHARGE}</h3>
	 </c:forEach>
...
<%-- <% 
	...
	for(int i =0; i<list.size(); i ++){
		StoreInfoDTO dto = (StoreInfoDTO)list.get(i);
		dto.getSEQ_PRODUCT();
		dto.getSEQ_GROUP();
		dto.getPRODUCT_NAME();
		dto.getDESCRIPTION();
		dto.getPRODUCT_CHARGE();
	}
	%>
 --%>
