...
$(function(){
	...
	$(form).submit(function(e){
		...
		$.ajax({
			...
			success: function(data){
				console.log("success");
				console.log(data);
				console.log(data.map.comments.nickname);
 ...
 function showHtml(data) {
        ...
        $.each(data, function(index, item) {
        	html += "<div class='qna-answer-box'><div class='qna-answer-title'>";
...
$(function(){
	...
	var toggle3 = 1;
	$("#question_like_btn").on("click", function(){
		if (toggle3 ==  1) {
			$("#question_like_btn").attr("src","../beep_images/community-images/redlike.svg");
			toggle3 = 0;
		} else {
			$("#question_like_btn").attr("src","../beep_images/community-images/whitelike.svg");
			toggle3 = 1;
		}
	});
...
function likebtn(like_btn){
	var id = like_btn.id;
	var num = id.substring(7,11);
	var imageId = "#likeimage"+num;
	var image = $(imageId).prop("src");
	var ps_seq = id.substring(9,11);
	var toggle = image.substring(image.length-9,image.length-8);
	if(toggle=="e"){
		$(imageId).attr("src","../beep_images/community-images/redlike.svg");
		likeAjaxConn('/beepPro/QnA/likebtn.ajax?like=y&ps_seq='+ps_seq);
	}else if(toggle="d"){
		$(imageId).attr("src","../beep_images/community-images/whitelike.svg");
		likeAjaxConn('/beepPro/QnA/likebtn.ajax?like=n&ps_seq='+ps_seq);
	}
}
		...
		$.ajax({
			...
			success: function(data){
				...
				if(data.map.result==1){
					...
					/*likeout(data.map);*/
					$('#liketext'+data.map.ps_seq).text(data.map.likes);
				}
			},
			error: function(data){
