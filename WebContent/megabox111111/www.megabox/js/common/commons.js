...
function sessionAllow( setting ){
    ...
    if( sessionMsg != "" ){
        gfn_alertMsgBox(sessionMsg, function(){
...
var Movie = function() {
    ...
    var htmlData = '<li tabindex="0" class="no-img">' +
        ...
        '        <a href="#" class="button purple bokdMxBtn" data-no="movieNo" title="MX관 예매하기">MX</a>' +
        '    </div>' +
...
*/
            ...
            target.append(html);
            liTarget = target.find('li').eq(Number(target.find('li').size())-1);
            ...
            if(v.boutqTheabAt == 'Y') liTarget.find('.boutqScreen').show(); // 부티크관 상영 여부
            if(v.mxTheabAt == 'Y') liTarget.find('.mxScreen').show(); // MX관 상영 여부

            if(v.movieStatCd == 'MSC01' && v.mxTheabAt == 'Y' && v.bokdAbleYn == 'Y') liTarget.find('.movieStat3').show(); // 예매, MX 예매 가능
            else if(v.movieStatCd == 'MSC01' && v.bokdAbleYn == 'Y') liTarget.find('.movieStat4').show(); // 예매 가능
            else if(v.movieStatCd == 'MSC02' && v.mxTheabAt == 'Y' && v.bokdAbleYn == 'Y' && v.rfilmDeReal != null) liTarget.find('.movieStat3').show(); // 예매, MX 예매 가능
    ...
    var addMouseEvent = function(target, myLikeAt) {
        ...
        // MX 예매 클릭
        $('.bokdMxBtn').off().on('click', function(e) {
          e.preventDefault();
            ...
            $.ajaxMegaBox({
                 ...
                 success: function(d) {
                     if(d.result == 'admisY') {
                         ...
                         options.callback = fn_callbackInsertMovies;
...
//본영화 등록 후 콜백함수
function fn_callbackInsertMovies(){
    $('#movie_regi').val('');
...
var Share = function() {
    ...
    this.facebook = function(option) {
        if (MegaboxUtil.Common.isApp()) {
            //도메인 주소 필수
            if( $("#fbUrl").attr("content").indexOf('index.html') == 0 ) {
            ...
            success       : function (data, textStatus, jqXHR) {
                ...
                if(data.mbNoticeList != undefined && data.mbNoticeList.length > 0) {
                    ...
                    for(var i in data.mbNoticeList) {
                        ...
                        if(rowData.notiLinkUrl.split("index.html")[1] == "mobile-order"){
